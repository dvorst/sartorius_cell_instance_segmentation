...
def to_annotation(img_data: pd.DataFrame, idx: str):
	...
	# plt.imshow(annotations[1],cmap='gray')
	# plt.show()
...
def decompress_single(data: str, width, height):
...
def _edges(annotations: torch.tensor):
	...
	dx = torch.empty((n, y, x + 2), dtype=torch.int16)  # empty() is faster than zeros()
	dy = torch.empty((n, y + 2, x), dtype=torch.int16)  # empty() is faster than zeros()
	dx[:, :, 1:-1] = annotations
	...
	edges = (
			(dx[:, :, :-2] - dx[:, :, 1:-1]).clamp(min=0, max=1) +  # right
			(dx[:, :, 2:] - dx[:, :, 1:-1]).clamp(min=0, max=1) +  # left
			(dy[:, :-2, :] - dy[:, 1:-1, :]).clamp(min=0, max=1) +  # bottom
			(dy[:, 2:, :] - dy[:, 1:-1, :]).clamp(min=0, max=1)  # top
	).clamp(min=0, max=1)
	return edges
...
def _mask(annotations, touch):
	...
	or 1.
	"""
	mask = torch.sum(annotations, dim=0, dtype=torch.int16)
