...
class SEBlock(nn.Module, ABC):  # OK
...
class SpatialAttBlock(nn.Module, ABC):  # OK
	def __init__(self, in_ch, med_ch):
		super().__init__()
		self.block = nn.Sequential(
			nn.Conv2d(
				in_ch, med_ch, kernel_size=(1, 1), stride=(1, 1),
				padding=(0, 0), bias=True
			),
			nn.BatchNorm2d(med_ch),
			nn.ReLU(),
			nn.Conv2d(
				med_ch, 1, kernel_size=(1, 1), stride=(1, 1),
				padding=(0, 0), bias=True
			),
			nn.Sigmoid())
...
class ResBlock(nn.Module, ABC):  # OK
	def __init__(self, in_ch):
		super().__init__()
		self.block = nn.Sequential(
			nn.Conv2d(
				in_ch, in_ch, kernel_size=(3, 3), stride=(1, 1),
...
class DualAttBlock(nn.Module, ABC):  # OK
	def __init__(self, skip_in_ch, prev_in_ch, out_ch):
		...
		self.block = nn.Sequential(
			nn.Conv2d(
				skip_in_ch + out_ch, out_ch, kernel_size=(3, 3),
				stride=(1, 1), padding=(1, 1), bias=True
			),
			nn.BatchNorm2d(out_ch),
			nn.ReLU())
		self.se_block = SEBlock(out_ch, ratio=16)
...
class GSCBlock(nn.Module, ABC):
	def __init__(self, in_ch_x, in_ch_y):
		super().__init__()
		self.block = nn.Sequential(
			nn.BatchNorm2d(in_ch_x + in_ch_y),
			nn.Conv2d(
				in_ch_x + in_ch_y,
				in_ch_x + in_ch_y + 1,
...
class ShapeStream(nn.Module, ABC):
	...
	def forward(self, x, res2, res3, res4, grad):
		size = grad.size()[-2:]
		x = func.interpolate(x, size, mode='bilinear', align_corners=True)
		res2 = func.interpolate(
			self.res2_conv(res2), size, mode='bilinear',
			align_corners=True
		)
		res3 = func.interpolate(
			self.res3_conv(res3), size, mode='bilinear',
			align_corners=True
		)
		res4 = func.interpolate(
			self.res4_conv(res4), size, mode='bilinear',
			align_corners=True
		)
...
class MSRF(nn.Module, ABC):
	...
	def forward(self, x, canny):
		...
		# SHAPE STREAM
		#    https://github.com/leftthomas/GatedSCNN
